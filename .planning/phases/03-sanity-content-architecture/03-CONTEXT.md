...
- **D-05:** **Structured price fields on `service`** — numeric price + currency + display label + a what's-included list, PLUS a **`priceOnRequest` boolean** and an **optional/nullable numeric price** (so a tier can show "price on request" with NO number while keeping the field in the schema for later). NOT a single free-text price string. Machine-readable and JSON-LD-ready for Phase 6. Avoids hardcoding prices in components. Rejected free-text price string (not machine-readable, duplicates formatting into content).
  - **RESOLVED PRICING (user decision 2026-08-12 — supersedes ROADMAP's illustrative "~€690 / ~€2,500" and reconciles the `freelancer.md` "keine fixen Pakete" conflict):** The studio now delivers full-service (concept → design → development → marketing); prices are raised to reflect that ROI.
    - **Landing tier:** concrete anchor **`ab €1.500`** — displayed as a "from" price (`priceFrom` semantics; label prefix "ab"). `price = 1500`, `currency = EUR`, `priceOnRequest = false`.
    - **Multi-page / full-site tier:** **"price on request"** — the tier IS shown (name, scope, what's-included) but with a CTA-style label (e.g. "Individuelles Angebot — nach Erstgespräch" / "Preis nach Erstgespräch"), NO published number. `priceOnRequest = true`, numeric price left null. Schema keeps the optional price field so a number can be added later without a schema migration.
  - **Display convention:** the "from/ab" anchor is a first-class rendering concern — the schema must carry enough (numeric value + a `priceFrom`/`priceOnRequest` flag + currency) for Phase 4 to render "ab €1.500" and "Preis nach Erstgespräch" without hardcoding. This softens SEC-03's "fixed pricing prominently displayed" to "concrete entry anchor + consultative full-site pricing" — an intentional, user-approved deviation; note it in the plan.
...
### Toolchain
- **D-12:** **Upgrade local Node to ≥22.12 as the FIRST plan task** (gated before Sanity install / Studio dev / `sanity typegen` / `sanity dataset import`). Rationale: research VERIFIED the machine runs Node v20.17.0 but `sanity@6.9.2` requires Node ≥22.12 — Studio, typegen, and dataset import cannot run locally otherwise, and the phase's success criteria (Studio shows schema types, content in Sanity, typed queries) can't be verified without it. User chose to bump the local toolchain now rather than author code and defer runtime verification. Also verify the Vercel runtime is ≥22 for deploy. Rejected "author schema/content code, defer runtime verify" (leaves success criteria unverifiable this phase).
...
- **Content migration mechanism:** manual authoring in Studio vs a seed/import script (`@sanity/client` + NDJSON) to populate the canonical content — planner's call (research recommends NDJSON + `sanity dataset import`). Either way, Sanity ends as the single source of truth.
- **Authoritative content source:** since v1 sources aren't in-repo, use `~/Documents/claude-contexts/freelancer.md` (identity, contact, maintenance terms) and the ROADMAP's stated copy (3 outcome-anchored testimonials, positioning) as the canonical content to author. **Pricing is now RESOLVED (see D-05) — no longer an open reconciliation:** landing = `ab €1.500`, multi-page = price on request. Do NOT author `freelancer.md`'s old €35/h or €450 Freundespreis figures as public pricing.
